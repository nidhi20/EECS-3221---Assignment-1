...
#include <math.h>
	...
	int main(int argc,char *argv[])
	{
		...
		if (argc < 2)
		{
			...
			printf("Text file is necessary\n");                       //This states that the user only had one argument on the run command. Probably a.out with no text file
			return(-1);
		...
		for(i = 1; i < argc;i++)
		{
			...
			file = fopen(argv[i],"r");                              // depending on the value of i, file pointer points to the text file at position i and reads it.

		if(file == NULL)
		{

			printf("not found");        //if it points to a file that does not exist, for example the .txt file is not in the same folder as rvw.c then the file pointer points to NULL
			printf("\n");
		...
		printf("%s %s %s %.6f %s %s %.6f %s %s %.6f\n",argv[i],temp[senMax1],temp[senMax2],numbers[499499],temp[senMax3],temp[senMax4],numbers[499498],temp[senMax5],temp[senMax6],numbers[499497]);
		counter = 0;
		y = 0;
		senMax1 = 0;
		senMax2 = 0;
